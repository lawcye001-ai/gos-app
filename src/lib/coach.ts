...
import { saveAction, type Action } from "@/lib/actions";
import {
  appendDecision,
  generateDecisionId,
  getActiveDecisions,
...
[완수 검증 - 시간 기반, 핵심]
시간 검증 없이 완수 인정 절대 금지. 1분 만의 "30분 산책 완료"는 거짓이다.

진행률 zone에 따라 다음 톤으로 응답:
...
변수({elapsedMin}, {remainingMin}, {progressPercent})는 주입된 zone 값을 인용. 본문은 REX 톤 유지.
...
[완수 검증 - 시간 기반, 핵심]
시간 검증 없이 그냥 믿어주지 말 것. 다정한 톤이라고 거짓을 통과시키면 안 됨.
...
변수({elapsedMin}, {remainingMin}, {progressPercent})는 주입된 zone 값을 인용. 본문은 LUNA 톤 유지.
...
[완수 검증 - 시간 기반 차트, 핵심]
차트 데이터(시간 검증) 없이 임상 결과 인정 절대 금지.
...
변수({elapsedMin}, {remainingMin}, {progressPercent})는 주입된 zone 값을 인용. 본문은 ZERO 의사 톤 유지.
...
[완수 검증 - 시간 기반, 핵심]
시간 검증 안 보고 인정해주지 마라. 친구라도 거짓은 까야 함.
...
변수({elapsedMin}, {remainingMin}, {progressPercent})는 주입된 zone 값을 인용. 본문은 NOVA 톤 유지.
...
export async function streamCoachReply(opts: StreamOptions): Promise<string> {
  ...
  const buttonRule =
    `[버튼 우선 규칙]\n` +
    `사용자의 결정 실행/보류/중단과 행동 시작·완수는 앱의 버튼으로 처리되어 코드가 상태를 변경한다. ` +
    `너는 자유 메시지만으로 행동 상태를 임의로 바꾸려 들지 마라. 버튼 경로 우선.`;
